/**
 * main_nvs.c - Zephyr NVS（键值存储）组件在模拟基座上的运行验证
 *
 * 定位：NVS 是 Zephyr 的 ID+数据键值存储（扇区式 ATE 日志 + 掉电安全 +
 * GC），vendor 零修改，经 Zephyr 兼容层（zephyr_compat）桥接 flash_sim。
 * 本测试验证：写入/读取、更新覆盖、删除、掉电安全、GC、功能压测。
 *
 * 测试驱动（环境变量，与平台其它框架保持一致）：
 *   SIM_*        模拟基座参数
 *   KV_CAPACITY  NVS 分区容量(字节)，须为擦除块整数倍且 >= 2 块
 *   KV_TESTS     测试项（write_read/update/delete/powerloss/gc/func）
...
/* 重新挂载（模拟设备重启后重新扫描恢复） */
static int nvs_remount(void)
{
    return nvs_mount(&s_fs);
}

static int nvs_put(uint16_t id, const void *val, size_t len)
{
    return nvs_write(&s_fs, id, val, len) >= 0 ? 0 : -1;
}

static int nvs_get(uint16_t id, void *val, size_t *len)
{
    ssize_t rc = nvs_read(&s_fs, id, val, *len);
...
{
    const char *v = "hello-zephyr-nvs";
    expect("write_read: write", nvs_put(0x0001, v, strlen(v)) == 0);

    char rb[64] = {0};
    size_t rl = sizeof(rb);
    expect("write_read: read", nvs_get(0x0001, rb, &rl) == 0);
...
{
    const char *v1 = "v1";
    nvs_put(0x0002, v1, strlen(v1));
    const char *v2 = "updated-value-longer-zephyr";
    expect("update: 再写", nvs_put(0x0002, v2, strlen(v2)) == 0);

    char rb[64] = {0};
    size_t rl = sizeof(rb);
    nvs_get(0x0002, rb, &rl);
    expect("update: 读到最新值",
...
static void t_delete(void)
{
    ...
    nvs_put(0x0003, &num, sizeof(num));
    expect("delete: del", nvs_delete(&s_fs, 0x0003) == 0);

    char rb[8] = {0};
    size_t rl = sizeof(rb);
    expect("delete: 删除后读不到", nvs_get(0x0003, rb, &rl) != 0);
}
...
{
    const char *keep = "must-survive-powerloss";
    expect("powerloss: 写入", nvs_put(0x0004, keep, strlen(keep)) == 0);
    ...
    expect("powerloss: 重启重载", nvs_remount() == 0);

    char rb[64] = {0};
    size_t rl = sizeof(rb);
    nvs_get(0x0004, rb, &rl);
    expect("powerloss: 已提交数据完好",
           rl == strlen(keep) && memcmp(rb, keep, rl) == 0);
}

/* GC：反复写同一条目使写入量远超分区容量，NVS 应自动触发扇区回收 */
static void t_gc(uint32_t capacity)
{
    ...
    for (uint32_t i = 0; i < iters; i++) {
        big[0] = (uint8_t)i;
        if (nvs_put(0x0005, big, sizeof(big)) != 0) { err++; }
    }
    expect("gc: 反复写入无失败", err == 0);

    uint8_t rb[128] = {0};
    size_t rl = sizeof(rb);
    nvs_get(0x0005, rb, &rl);
    expect("gc: GC 后数据正确",
...
static void t_types(void)
{
    ...
    nvs_put(0x0010, &iv, sizeof(iv));
    nvs_put(0x0011, &fv, sizeof(fv));
    nvs_put(0x0012, sv, strlen(sv));

    int32_t iv2 = 0;
    float fv2 = 0.0f;
    char sv2[32] = {0};
    size_t saved = sizeof(iv2);
    nvs_get(0x0010, &iv2, &saved);
    expect("types: int", iv2 == iv);
    saved = sizeof(fv2);
    nvs_get(0x0011, &fv2, &saved);
    expect("types: float", fv2 == fv);
    saved = sizeof(sv2);
    nvs_get(0x0012, sv2, &saved);
    expect("types: string",
                      ...
                      uint32_t *acc_ops, uint32_t *acc_lost)
{
    ...
    for (uint32_t k = 1; k <= n; k++) {
        ...
        if (nvs_put((uint16_t)k, buf, vlen) == 0) { (*acc_ops)++; }
    }
    ...
    for (uint32_t r = 0; r < rounds; r++) {
        for (uint32_t k = 1; k <= n; k++) {
            if ((rnd() % 100u) < freq) {
                for (uint32_t j = 0; j < vlen; j++) { buf[j] = (uint8_t)rnd(); }
                if (nvs_put((uint16_t)k, buf, vlen) != 0) {
                    lost++;
                    continue;
                }
                (*acc_ops)++;
                size_t rl = vlen;
                if (nvs_get((uint16_t)k, rbuf, &rl) != 0 ||
                    rl != vlen || memcmp(buf, rbuf, vlen) != 0) { lost++; }
            } else {
                size_t rl = vlen;
                if (nvs_get((uint16_t)k, rbuf, &rl) == 0) { (*acc_ops)++; }
            }
        }
    }
...
int main(void)
{
    ...
    if (cfg.type == FLASH_TYPE_EEPROM) {
        printf("  [info] NVS 依赖块擦除语义，EEPROM 自动切换为 NOR\n");
    ...
    printf("  [info] 介质=%s 容量=%uB 块=%uB | NVS 区=%uB (%u 块)\n",
    ...
    if (nvs_mount(&s_fs) != 0) {
        printf("  [FAIL] nvs_mount 失败\n");
        flash_sim_deinit(s_dev);
        return 1;
    }
    printf("  [OK  ] nvs_mount 初始化成功\n");
