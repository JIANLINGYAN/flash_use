 ...
 * 本移植层把这些契约函数对接到 simulator/flash_sim.c 的统一 Flash 抽象，
 * vendor/ 源码零修改。
 *
...
/**
 * 注入 NVDM 分区配置（须在 nvdm_init 之前调用）。
 * @param dev        模拟基座设备句柄
 * @param base_addr  分区起始偏移（相对 flash_sim 介质，建议 0）
 * @param capacity   分区容量（字节），须为 peb_size 整数倍且 >= 2*peb_size
 * @param peb_size   物理擦除块大小（须等于 flash 的 erase_size）
