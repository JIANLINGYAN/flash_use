...
#define   NVDM_FREE_ALERT_SIZE  4096
...
void peb_print_info(uint32_t num)
{
    ...
    nvdm_log_msgid_warning(nvdm_131, 1, p_info->curr_item_count);
    ...
    if (p_info->curr_item_count > (uint32_t)(p_cfg->total_item_count * 0.9)) {
        nvdm_log_msgid_error(nvdm_132, 0);
    }
    if ((p_info->avaliable_space - p_info->valid_data_size) < NVDM_FREE_ALERT_SIZE) {
        nvdm_log_msgid_error(nvdm_133, 1, NVDM_FREE_ALERT_SIZE);
    }
}
 ...
 */
#define NVDM_MAX_USAGE_RATIO 90
static uint32_t calculate_total_avail_space(uint32_t partition)
{
    ...
    criteria2 = (p_cfg->peb_count - NVDM_RESERVED_PEB_COUNT) *
                (p_cfg->peb_size - PEB_HEADER_SIZE - (p_cfg->max_item_size >> 1) - DATA_ITEM_HEADER_SIZE - DATA_ITEM_CHECKSUM_SIZE - p_cfg->max_group_name_size - p_cfg->max_item_name_size - 2);
...
nvdm_status_t nvdm_query_space_information(uint32_t partition, nvdm_space_info_t *info)
{
    ...
    info->item_count = p_info->curr_item_count;
