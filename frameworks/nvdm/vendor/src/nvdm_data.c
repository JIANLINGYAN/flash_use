...
static int32_t cmp_item_data_in_buffer_and_flash(uint32_t partition, const uint8_t *buffer, uint32_t buff_sz, const data_item_header_t *header)
{
    ...
    int32_t i, offset, fragment, remain, cmp_res = 0;
    uint8_t *working_buffer = g_ncb.working_buffer;
    uint32_t cmp_len;
    ...
    fragment = buff_sz / NVDM_BUFFER_SIZE;
    remain   = buff_sz % NVDM_BUFFER_SIZE;
    for (i = 0; i < fragment; i++) {
    ...
    if (remain) {
        peb_read_data(partition, header->pnum, offset, working_buffer, remain);
        cmp_len = remain;
        cmp_res = memcmp(working_buffer, buffer, cmp_len);
        if (cmp_res != 0) {
            return cmp_res;
                                                ...
                                                const void *user_data)
{
    ...
    lr_addr = (uint32_t)__builtin_return_address(0);
    task_name = nvdm_port_get_curr_task_name();

#if MARK_UNUSED_LOG_INFO_CALL
    nvdm_log_msgid_info(nvdm_022, 0);
#endif

    /* If task not started, user should not using this API.
     * Because this API need deamon task process*/
    if (strncmp(task_name, "main", 4) == 0) {
        nvdm_log_msgid_error("[nvdm] nvdm_write_data_item_non_blocking fail: task not started(caller:0x%x)", 1, lr_addr);
        return NVDM_STATUS_ERROR;
    }
        ...
        if (checksum1 != checksum2) {
            ...
            if (data_item_header.group_name_size + data_item_header.data_item_name_size <= NVDM_BUFFER_SIZE) {
                ...
                if ((data_item_header.group_name_size + data_item_header.data_item_name_size) < NVDM_BUFFER_SIZE) {
                    ...
                    g_ncb.working_buffer[NVDM_BUFFER_SIZE - 1] = '\0';
                }
